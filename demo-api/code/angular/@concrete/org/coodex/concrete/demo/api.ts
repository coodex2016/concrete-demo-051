...
export class TokenDemoService extends AbstractConcreteService {
    ...
    public setTokenValue(value: string): Observable<void> {
        return this.http.request('PUT', TokenDemoService.$$getServiceRoot() + `/OrgCoodexConcreteDemoApiTokenDemoService/tokenValue`, TokenDemoService.defaultRequestOptions({ value: value }))
                .pipe(map(TokenDemoService.extractData), catchError(TokenDemoService.handleError));
    }
}
...
export class AddWithIn10Service extends AbstractConcreteService {
    ...
    public add(x1: number, x2: number): Observable<number> {
        return this.http.request('POST', AddWithIn10Service.$$getServiceRoot() + `/OrgCoodexConcreteDemoApiAddWithIn10Service/add`, AddWithIn10Service.defaultRequestOptions({ x1: x1, x2: x2 }))
...
export class AddService extends AbstractConcreteService {
    ...
    public add(x1: number, x2: number): Observable<number> {
        return this.http.request('POST', AddService.$$getServiceRoot() + `/Calculator/add`, AddService.defaultRequestOptions({ x1: x1, x2: x2 }))
                .pipe(map(AddService.extractData), catchError(AddService.handleError));
    }
}

@Injectable()
export class ErrorCodeService extends AbstractConcreteService {

    constructor(private http: HttpClient) {
        super();
    }

    public freeMarker(arg0: CarInfo): Observable<void> {
        return this.http.request('POST', ErrorCodeService.$$getServiceRoot() + `/OrgCoodexConcreteDemoApiErrorCodeService/freeMarker`, ErrorCodeService.defaultRequestOptions(arg0))
                .pipe(map(ErrorCodeService.extractData), catchError(ErrorCodeService.handleError));
    }
    public noneAnnotation(): Observable<void> {
        return this.http.request('GET', ErrorCodeService.$$getServiceRoot() + `/OrgCoodexConcreteDemoApiErrorCodeService/noneAnnotation`, ErrorCodeService.defaultRequestOptions())
                .pipe(map(ErrorCodeService.extractData), catchError(ErrorCodeService.handleError));
    }
    public templateAnnotation(): Observable<void> {
        return this.http.request('GET', ErrorCodeService.$$getServiceRoot() + `/OrgCoodexConcreteDemoApiErrorCodeService/templateAnnotation`, ErrorCodeService.defaultRequestOptions())
                .pipe(map(ErrorCodeService.extractData), catchError(ErrorCodeService.handleError));
    }
}
...
export class SubjoinExampleService extends AbstractConcreteService {
    ...
    public add(x1: number, x2: number): Observable<number> {
        return this.http.request('POST', SubjoinExampleService.$$getServiceRoot() + `/SubjoinExample/add`, SubjoinExampleService.defaultRequestOptions({ x1: x1, x2: x2 }))
    ...
    public sayHello(name: string): Observable<string> {
        return this.http.request('POST', SubjoinExampleService.$$getServiceRoot() + `/SubjoinExample/sayHello`, SubjoinExampleService.defaultRequestOptions({ name: name }))
                .pipe(map(SubjoinExampleService.extractData), catchError(SubjoinExampleService.handleError));
    }
}
